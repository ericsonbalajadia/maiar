...
import { Paperclip, User, Calendar, MapPin, Building, Mail, Wrench, CheckCircle2, ClipboardList, ChevronRight, Home } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center text-sm text-slate-500">
        ...
        <Link href="/admin/requests" className="hover:text-slate-700">
          All Requests
        </Link>
        <ChevronRight className="h-3.5 w-3.5 mx-1" />
        <span className="font-medium text-slate-700">{request.ticket_number}</span>
      </div>
      ...
      {request.request_type === 'rmr' && rmr && (rmr.inspection_date || rmr.inspector_notes) && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2">
            ...
            {rmr.materials_available !== null && <InfoRow label="Materials" value={rmr.materials_available ? 'Available' : 'Not available'} />}
            {rmr.manpower_required !== null && <InfoRow label="Manpower" value={`${rmr.manpower_required} person(s)`} />}
          </div>
          {rmr.inspector_notes && (
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
        ...
        {attachments.length === 0 ? (
          ...
          <AttachmentPreview attachments={attachments} requestId={id} canDelete={true} />
        )}
      </div>
