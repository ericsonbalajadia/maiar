  ...
  const inner = (
    ...
    >
      ...
      <div className="flex items-start gap-3 p-4">
        <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center shrink-0', meta.bg, 'border', isRequester ? 'border-[#ADEBB3]/70 dark:border-white/10' : meta.border)}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2.5 flex-wrap">
            ...
            {notif.requests?.ticket_number && href && (
              <Link
                href={href}
                className="inline-flex items-center gap-1 text-[11px] font-mono text-slate-500 dark:text-white/60 hover:text-[#527255] dark:hover:text-emerald-300 transition-colors"
            ...
            {isUnread && (
              <button onClick={() => onRead(notif.id)} className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 hover:text-[#527255] transition-colors ml-auto">
                Mark as read
              </button>
            )}
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 fade-in">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className={cn('text-xs font-semibold uppercase tracking-widest mb-1', isRequester ? 'text-[#527255] dark:text-emerald-300' : 'text-blue-500 dark:text-blue-400')}>Notifications</p>
        ...
        {unreadCount > 0 && (
          <Button variant="outline" size="sm" onClick={handleMarkAll} disabled={isPending}
            className={cn('gap-1.5 shrink-0', isRequester ? 'text-[#1e2c1f] border-[#ADEBB3]/70 dark:border-white/10 hover:bg-[#ADEBB3]/35 dark:hover:bg-white/[0.08]' : 'text-blue-600 border-[#ADEBB3]/70 dark:border-white/10 hover:bg-[#ADEBB3]/35 dark:hover:bg-white/[0.08]')}>
            <CheckCheck className="h-3.5 w-3.5" /> Mark all read
          </Button>
        )}
      </div>

      <div className={cn('flex items-center gap-1 p-1 rounded-xl border w-fit', isRequester ? 'border-[#ADEBB3]/70 dark:border-white/10' : 'border-[#ADEBB3]/70 dark:border-white/10')} style={{ background: 'var(--glass-bg)', backdropFilter: 'blur(12px)' }}>
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div key={i} className={cn('rounded-xl border p-4 flex gap-3 bg-white/60 dark:bg-white/[0.04]', isRequester ? 'border-[#ADEBB3]/70 dark:border-white/10' : 'border-[#ADEBB3]/70 dark:border-white/10')}>
        ...
        <div className={cn('rounded-2xl border flex flex-col items-center justify-center py-16 text-center', isRequester ? 'border-[#ADEBB3]/70 dark:border-white/10' : 'border-[#ADEBB3]/70 dark:border-white/10')} style={{ background: 'var(--glass-bg)', backdropFilter: 'blur(12px)' }}>
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-700 flex items-center justify-center mb-4">
            {filter === 'unread' ? <CheckCheck className="h-6 w-6 text-[#8dc192]" /> : <Inbox className="h-6 w-6 text-slate-400" />}
          </div>
