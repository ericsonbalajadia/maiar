...
export function AssignTechnicianForm({ requestId, technicians, currentAssignedId, ticketNumber }: Props) {
  const [state, formAction] = useActionState(assignTechnician, { success: false });
  const [isPending, startTransition] = useTransition();

  const sorted = [...technicians].sort((a, b) => a.active_assignments - b.active_assignments);
  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-3">
      ...
      {state.success && (
        <div className="text-xs text-green-600">✓ Technician assigned successfully.</div>
      )}
      ...
      <form action={(fd) => startTransition(() => formAction(fd))} className="space-y-3">
        ...
        <div>
          <label htmlFor="technicianId" className="block text-sm font-medium text-muted-foreground mb-1">
            Technician
          </label>
          <select
            name="technicianId"
        ...
        <div>
          <label htmlFor="notes" className="block text-sm font-medium text-muted-foreground mb-1">
            Assignment Notes <span className="text-muted-foreground/60">(optional)</span>
          </label>
          <Textarea
            id="notes"
            name="notes"
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="submit" disabled={isPending} variant="default" size="sm" className="bg-teal-600 hover:bg-teal-700 text-white">
