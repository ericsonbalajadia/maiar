    ...
    <div className="grid grid-cols-5 gap-2 w-full">
      {stars.map((star) => (
        <label key={star} className="flex flex-col items-center gap-1 cursor-pointer">
          ...
          />
          <span className="h-8 w-8 flex items-center justify-center rounded-full border border-[#ADEBB3]/70 text-sm text-slate-600 peer-checked:border-[#ADEBB3] peer-checked:bg-[#ADEBB3]/30 peer-checked:text-[#1e2c1f] peer-checked:font-bold hover:border-[#ADEBB3]/70 transition-colors">
            {star}
          </span>
          {labels && (
